# ================================================================
# wheatspec_app.py  —  WheatSpec v3 (Times New Roman + Dark Theme)
...
# ================================================================
# CSS  — Times New Roman + Permanent Midnight Dark Premium Theme
# ================================================================
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;600&display=swap');

/* Force Times New Roman globally across all Streamlit container frameworks */
...
.hero h1 { font-size: 2.3rem; font-weight: 700; margin: 0 0 6px 0; color: #FFFFFF !important; }
.hero p { font-size: 1.05rem; opacity: 0.85; margin: 0; line-height: 1.5; color: #E2E8F0 !important; }
.hero-chips { margin-top: 14px; display: flex; gap: 8px; flex-wrap: wrap; }
.chip { background: #334155; border: 1px solid #475569;
        border-radius: 4px; padding: 3px 12px; font-size: 0.8rem; color: #F1F5F9; }

/* ── High-Contrast Dashboard Metric Cards ── */
.mcard { background: #1E293B; border: 1px solid #334155; border-radius: 8px;
         padding: 1.2rem 1.4rem; margin-bottom: 0.8rem;
         border-left: 4.5px solid #22C55E; box-shadow: 0 4px 10px rgba(0,0,0,0.15); }
.mcard.blue { border-left-color: #3B82F6; }
.mcard.amber { border-left-color: #F59E0B; }
.mcard.purple { border-left-color: #8B5CF6; }
.mlabel { font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.08em;
          color: #94A3B8; font-weight: 600; margin-bottom: 6px; }
.mvalue { font-size: 1.9rem; font-weight: 700; color: #FFFFFF;
          font-family: 'JetBrains Mono', monospace !important; line-height: 1.1; }
.msub { font-size: 0.85rem; color: #94A3B8; margin-top: 5px; }
.mdiff { font-size: 0.85rem; margin-top: 4px; font-weight: 600; }

/* ── Status and Evaluation Badges ── */
.badge { display: inline-block; padding: 2px 8px; border-radius: 4px;
         font-size: 0.75rem; font-weight: 700; text-transform: uppercase; margin-left: 5px; }
.bexc  { background: #14532D; color: #BBF7D0; }
.bqc   { background: #1E3A8A; color: #BFDBFE; }
.bscr  { background: #713F12; color: #FEF08A; }
.bpoor { background: #7F1D1D; color: #FECACA; }

/* ── Context Notification Boxes ── */
.kbox { background: #14532D; border: 1px solid #16A34A; border-radius: 6px;
        padding: 0.8rem 1.1rem; font-size: 0.95rem; color: #EFF6FF; margin: 0.8rem 0; }
.wbox { background: #78350F; border: 1px solid #D97706; border-radius: 6px;
        padding: 0.8rem 1.1rem; font-size: 0.95rem; color: #FEF3C7; margin: 0.8rem 0; }
.ibox { background: #1E3A8A; border: 1px solid #2563EB; border-radius: 6px;
        padding: 0.8rem 1.1rem; font-size: 0.95rem; color: #EFF6FF; margin: 0.8rem 0; }

/* ── Publication Section Labels ── */
.slabel { font-size: 0.85rem; text-transform: uppercase; letter-spacing: 0.08em;
          color: #94A3B8; font-weight: 700; margin: 1.6rem 0 0.6rem; }

/* ── Sidebar & Controls Overrides ── */
div[data-testid="stSidebarContent"] { background: #1E293B !important; border-right: 1px solid #334155; }
div[data-testid="stSidebarContent"] .stMarkdown p, div[data-testid="stSidebarContent"] label { color: #E2E8F0 !important; }
.stTabs [data-baseweb="tab-list"] { gap: 10px; background-color: #0F172A; padding: 4px; border-radius: 8px; }
.stTabs [data-baseweb="tab"] { background-color: #1E293B; border: 1px solid #334155; border-radius: 6px; padding: 0.5rem 1.4rem; color: #94A3B8; }
...
with st.sidebar:
    ...
    st.caption("Name | low cm⁻¹ | high cm⁻¹ — one per line. Edit freely.")
    region_raw = st.text_area("Regions",
    ...
    st.caption("Trait | R² — one per line. Leave blank to skip comparison.")
    bench_raw = st.text_area("Benchmarks",
        "Protein | 0.963\nExtensibility | 0.927\n"
        "Absorption | 0.700\nRmax | 0.482\nDDT | 0.500",
        height=140)
    ...
    excl_raw = st.text_input("Non-trait columns (comma-separated)",
                              "Variety,variety,Cultivar,cultivar,Sample,sample,ID,id,Name,name,Seed,seed")
    EXCL = {c.strip().lower() for c in excl_raw.split(",")}

    st.markdown("---")
    st.markdown("## 🎨 Cultivar colours")
    st.caption("Class | hex colour — one per line. Auto-applied to LDA plot.")
    colour_raw = st.text_area("Colours",
        "MACE | #1E40AF\nSCEPTER | #3B82F6\nCORACK | #0D9488\n"
        "MAGENTA | #9D174D\nEMU_ROCK | #D97706\nZEN | #16A34A",
        height=160)
...
with tab1:
    ...
    if df is None:
        st.markdown('<div class="ibox">📁 Upload a spectral matrix in the sidebar to begin.</div>',
                    unsafe_allow_html=True)
    elif len(wave_cols) < 5:
        st.error("Fewer than 5 numeric wavenumber columns detected — check file format.")
    else:
        st.success(f"**{df.shape[0]} samples** · **{len(wave_cols)} spectral points** · "
                   f"**{len(trait_cols)} trait columns** detected")

        id_col = st.selectbox("Sample label column (hover labels in plot)",
                               ["(none)"] + non_wave)
        ...
        labels = df[id_col].astype(str).values if id_col else \
                 [f"Sample {i}" for i in range(len(df))]
        n_show = min(n_plot, len(df))

        # ── Raw vs Processed side by side ──
        ca, cb = st.columns(2)
        for col, data, title in [(ca, X_raw, "Raw spectra"),
                                 (cb, X_proc, "After preprocessing")]:
            with col:
                ...
                fig.update_layout(xaxis=dict(title="Wavenumber (cm⁻¹)",
                                              autorange="reversed", gridcolor="#334155"),
                                   yaxis_title="Intensity", height=330,
        ...
        # ── Region overlay ──
        if REGIONS:
            st.markdown('<p class="slabel">Kelly spectral regions overlay</p>',
                        unsafe_allow_html=True)
            palette = px.colors.qualitative.Pastel
            fig_r = go.Figure()
            for i in range(n_show):
                fig_r.add_trace(go.Scatter(x=wn, y=X_proc[i], mode="lines",
                                            line=dict(width=0.7, color="#94A3B8"),
                                            opacity=0.35, showlegend=False))
            for idx, (rn, (lo, hi)) in enumerate(REGIONS.items()):
                fig_r.add_vrect(x0=lo, x1=hi,
                                 fillcolor=palette[idx % len(palette)],
                                 opacity=0.2, layer="below", line_width=0,
                                 annotation_text=rn.split(" ")[0],
                                 annotation_position="top left",
                                 annotation_font_color="#FFFFFF", annotation_font_size=10)
            fig_r.update_layout(
                xaxis=dict(title="Wavenumber (cm⁻¹)", autorange="reversed", gridcolor="#334155"),
                yaxis_title="Processed intensity", height=370,
                template="plotly_dark", paper_bgcolor='#1E293B', plot_bgcolor='#1E293B',
                title="Preprocessed spectra — user-defined region boundaries",
                margin=dict(l=40,r=10,t=40,b=40))
            st.plotly_chart(fig_r, use_container_width=True)

        # ── Mean spectrum per class ──
        if id_col:
            st.markdown('<p class="slabel">Mean spectrum per class</p>',
                        unsafe_allow_html=True)
            ...
            for grp, sub in df_proc.groupby("_label"):
                ...
                fig_m.add_trace(go.Scatter(x=wn, y=mean_spec.values, mode="lines",
                                            name=str(grp), line=dict(width=1.8)))
            fig_m.update_layout(xaxis=dict(title="Wavenumber (cm⁻¹)", autorange="reversed", gridcolor="#334155"),
                                  yaxis_title="Mean intensity", height=320,
                                  template="plotly_dark", paper_bgcolor='#1E293B', plot_bgcolor='#1E293B',
...
with tab2:
    ...
    if df is None:
        st.markdown('<div class="ibox">📁 Upload data in the sidebar to begin.</div>',
                    unsafe_allow_html=True)
    elif not trait_cols:
        st.markdown('<div class="wbox">No trait columns detected. Adjust exclusion layers.</div>',
                    unsafe_allow_html=True)
    elif not REGIONS:
        st.markdown('<div class="wbox">No spectral regions defined.</div>',
                    unsafe_allow_html=True)
    elif not any([use_plsr, use_bayes, use_rf, use_xgb]):
        st.markdown('<div class="wbox">Select at least one model in the sidebar.</div>',
                    unsafe_allow_html=True)
    else:
        c1, c2 = st.columns(2)
        ...
        if bench:
            st.markdown(f'<div class="kbox">📖 <strong>Benchmark R² for {sel_trait}:</strong> '
                        f'{bench} — from your sidebar benchmark settings</div>',
                        unsafe_allow_html=True)
            ...
            if len(rc) < 3:
                ...
                if use_plsr:
                    with st.spinner("Optimising PLSR via LOO cross-validation…"):
                        ...
                        results.append({"Model": f"PLSR ({best_n} comp)", "color":"",
                                        "R²": round(r2_score(y_te, yp), 4),
                                        "RPD": round(rpd_v(y_te, yp), 3), "y_pred": yp})

                ml = build_ml(use_bayes, use_rf, use_xgb, rf_n, xgb_n,
                               xgb_lr, xgb_depth, n_pca, X_tr)
                ...
                for mname, pipe in ml.items():
                    with st.spinner(f"Fitting {mname}…"):
                        try:
                            ...
                            results.append({"Model": mname, "color": colors.get(mname,""),
                                            "R²": round(r2_score(y_te, yp), 4),
                                            "RPD": round(rpd_v(y_te, yp), 3), "y_pred": yp})
                ...
                for col, res in zip(cols, results):
                    ...
                    with col:
                        st.markdown(mcard(res["Model"], res["R²"], f"RPD {res['RPD']}",
                                          gr, bc, diff, res["color"]), unsafe_allow_html=True)

                # ── Predicted vs actual grid ──
                st.markdown('<p class="slabel">Predicted vs actual parity coordinates</p>',
                            unsafe_allow_html=True)
                nc  = len(results)
                ...
                for i, res in enumerate(results, 1):
                    fig.add_trace(go.Scatter(x=y_te, y=res["y_pred"], mode="markers",
                                              marker=dict(color=pal[(i-1)%len(pal)], size=7, opacity=0.85,
                    ...
                    fig.add_trace(go.Scatter(x=lims, y=lims, mode="lines",
                                              ...
                                              showlegend=False), row=1, col=i)
                fig.update_layout(height=310, template="plotly_dark", paper_bgcolor='#1E293B', plot_bgcolor='#1E293B',
                ...
                if rmsecv_curve:
                    bn, sc = rmsecv_curve
                    st.markdown('<p class="slabel">PLSR LOO RMSECV Latent Space Optimisation</p>',
                                unsafe_allow_html=True)
                    fc = px.line(x=list(range(1, len(sc)+1)), y=sc, markers=True,
                                 labels={"x":"Components","y":"RMSECV"}, color_discrete_sequence=["#22C55E"])
                    fc.add_vline(x=bn, line_dash="dash", line_color="#F59E0B",
                                 annotation_text=f"Optimal n={bn}", annotation_position="top right")
                    fc.update_layout(height=250, template="plotly_dark", paper_bgcolor='#1E293B', plot_bgcolor='#1E293B',
                                     margin=dict(l=40,r=10,t=20,b=40))
...
with tab3:
    ...
    if df is None:
        ...
        if st.button("▶ Run LDA classification", type="primary"):
            ...
            if upper_case:
                df_v[var_col] = df_v[var_col].astype(str).str.upper().str.strip()
            ...
            if nc < 2:
                ...
                prog.empty()
                if not records:
                    st.error("LDA failed on all tracking configurations.")
                else:
                    ...
                    st.markdown(f"""<div class="mcard" style="max-width:380px">
                        ...
                        <div class="mdiff" style="color:{c}">{"↑" if diff>=0 else "↓"} {diff:+.1%} vs benchmark ({lda_bench:.0%})</div>
                    </div>""", unsafe_allow_html=True)

                    # Heatmap
                    st.markdown('<p class="slabel">Accuracy Matrix — region × preprocessing</p>', unsafe_allow_html=True)
                    ...
                    if show_3d:
                        ...
                        if n_lda_comp >= 3:
                            fig3d = px.scatter_3d(df3d, x="LD1", y="LD2", z="LD3", color="Variety", color_discrete_map=colour_map, opacity=0.9)
                        else:
                            fig3d = px.scatter(df3d, x="LD1", y="LD2" if n_lda_comp>=2 else "LD1", color="Variety", color_discrete_map=colour_map, opacity=0.9)
...
with tab4:
    ...
    if df is None:
        ...
        st.warning("No parameters located in trait array columns.")
    elif not REGIONS:
        st.warning("No regional spectra slices defined.")
    elif not any([use_plsr,use_bayes,use_rf,use_xgb]):
        st.warning("Activate at least one validation core checkbox.")
    else:
        ...
        if st.button("▶ Launch tournament", type="primary") and sel_targets:
            ...
            for target in sel_targets:
                ...
                for rname, (lo, hi) in REGIONS.items():
                    ...
                    def _rec(mn, yp):
                        ...
                        gr, _ = rpd_badge(rpd)
                        return {"Target":target,"Region":rname,"Model":mn,"R²":r2,"RPD":rpd,"Grade":gr,
                                "Benchmark":bench, "vs_Benchmark":round(r2-bench,4) if bench else None}
            ...
            if not all_rec:
                st.error("Validation loop yielded zero output arrays.")
            else:
                res_df = pd.DataFrame(all_rec)

                # ── Best per trait cards ──
                ...
                for col, row in zip(cols, best_df.itertuples()):
                    ...
                    with col:
                        st.markdown(mcard(row.Target, row._5, f"{row.Model} · {str(row.Region)[:14]}", gr, bc, row.vs_Benchmark), unsafe_allow_html=True)
                ...
                for target in sel_targets:
                    ...
                    b = BENCH.get(target)
                    if b:
                        fig.add_hline(y=b, line_dash="dash", line_color="#F59E0B", annotation_text=f"Benchmark {b}", annotation_position="top right")
                ...
                def _highlight(row):
                    if row["R²"] == res_df["R²"].max():
                        return ["background:#14532D;font-weight:700;color:#22C55E"]*len(row)
                    return [""]*len(row)
...
<p style="font-size:0.82rem;color:#64748B;text-align:center;padding:0.5rem 0">
  WheatSpec v3 &nbsp;·&nbsp; Manuli Perera &nbsp;·&nbsp; UWA Dissertation 2026
  &nbsp;·&nbsp; Open-source MIT licence &nbsp;·&nbsp; Built on <em>Kelly et al. (2023) Food Chemistry</em>
